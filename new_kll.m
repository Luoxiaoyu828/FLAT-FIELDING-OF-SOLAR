function [ output_args ] = new_kll( input_args )
close all;clear all

% % a=[0 0;349   155; -368  127;-130 150;-130 -150;-200 -200;100 -200;250 -200;150 00];ks=9;%平移的坐标
% a=[0 0;0 30;0,-30;30 0;-30 0;0 50;0 -50;50 0;-50 0;70,0;-70,0];ks=11;%平移的坐标
...
I{1}=imresize(double(imread('h619l.jpg')),sc);
...
[m,n]=size(I{1}),
for i=1:9   %自动根据相关性求出平移坐标，对仿真图像不需要
...
for i=1:9   %根据最小二乘拟合求出平移坐标，对仿真图像不需要
    a(i,:) = round(algin(I{1},I{i})) ;
end
...
figure;imshow(O0,[]);%Log object of original data 
% TT=O-log(O0);TTS=TT.^2;psnr=10*log10(100./mean(TTS(:))), %JC method 2004 
