...
tic
itnumber=15;%迭代次数
radius=500;%模仿的太阳成像大小,取中间较均匀的区域进行平场估计
for i=1:ks
...
toc
figure,imshow(exp(G0),[]),title('MCC平场')
figure,imshow(exp(G1),[]),title('KLL平场')
figure,imshow(exp(D{1}-G0),[]),title('MCC目标')
figure,imshow(exp(D{1}-G1),[]),title('KLL目标')
figure,imshow(exp(O0),[]),title('原始目标')
return
